...
class LeaderboardEvaluator(object):
    ...
    def __init__(self, args, statistics_manager):
        ...
        if dist.version != 'leaderboard':
            if LooseVersion(dist.version) < LooseVersion('0.9.10'):
                raise ImportError(
                    "CARLA version 0.9.10.1 or newer required. CARLA version found: {}".format(dist))
        ...
        self._end_time = None
        # print("\033[1m> Start time: {}\033[0m".format(
        #     datetime.fromtimestamp(self._start_time).strftime('%Y-%m-%d %H:%M:%S')))
        # print("\033[1m> Timeout: {}s\033[0m".format(args.timeout))
    ...
    def _register_statistics(self, config, checkpoint, entry_status, crash_message=""):
        ...
        self.statistics_manager.save_entry_status(
            entry_status, False, checkpoint)
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            self.agent_instance = getattr(
                self.module_agent, agent_class_name)(args.agent_config)
            # self.agent_instance = getattr(
            #     self.module_agent, agent_class_name)(config)
            ...
            if not self.sensors:
                ...
                self.sensor_icons = [sensors_to_icons[sensor['type']]
                                     for sensor in self.sensors]
                self.statistics_manager.save_sensors(
                    self.sensor_icons, args.checkpoint)
        ...
        try:
            self._load_and_wait_for_world(
                args, config.town, config.ego_vehicles)
        ...
        str_command = 'python3 '+os.getcwd()+'/utilx/environmentx.py --weather ' + \
            args.weather+' --host '+args.host+' --port '+args.port
        print(str_command)
...
def main():
    ...
    parser.add_argument("--weather", type=str,
                        help="Weather yang diinginkan, lihat utilx/environmentx.py", default='ClearNoon')
