...
class BaseAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        if SAVE_PATH is not None:
            ...
            string = self.config_path.name + '_'
    ...
    def tick(self, input_data):
        ...
        for pos in ['front', 'left', 'right', 'rear']:
            ...
            self._change_seg_stop(
                _segmentation, depth[pos], stop_signs, seg_cam)
        ...
        depth_rear = cv2.cvtColor(
            input_data['depth_rear'][1][:, :, :3], cv2.COLOR_BGR2RGB)
    ...
    def save(self, near_node, far_node, near_command, steer, throttle, brake, target_speed, tick_data):
        ...
        measurements_file = self.save_path / \
            'measurements' / ('%04d.json' % frame)
        ...
        for pos in ['front', 'left', 'right', 'rear']:
            ...
            for sensor_type in ['2d_bbs']:
                name = sensor_type + '_' + pos
                np.save(self.save_path / name / ('%04d.npy' %
                        frame), tick_data[name], allow_pickle=True)

        Image.fromarray(tick_data['topdown']).save(
            self.save_path / 'topdown' / ('%04d.png' % frame))
        np.save(self.save_path / 'lidar' / ('%04d.npy' %
                frame), tick_data['lidar'], allow_pickle=True)
        np.save(self.save_path / 'semantic_lidar' / ('%04d.npy' %
                frame), tick_data['semantic_lidar'], allow_pickle=True)
        np.save(self.save_path / '3d_bbs' / ('%04d.npy' %
                frame), tick_data['3d_bbs'], allow_pickle=True)
        np.save(self.save_path / 'affordances' / ('%04d.npy' %
                frame), tick_data['affordances'], allow_pickle=True)
    ...
    def _find_obstacle_3dbb(self, obstacle_type, max_distance=50):
        ...
        for _obstacle in _obstacles:
            distance_to_car = _obstacle.get_transform(
            ).location.distance(self._vehicle.get_location())
            ...
            if 0 < distance_to_car <= max_distance:
                ...
                if hasattr(_obstacle, 'bounding_box'):
                    ...
                    _rotation_matrix = self.get_matrix(carla.Transform(
                        carla.Location(0, 0, 0), _obstacle.get_transform().rotation))

                    rotated_extent = np.squeeze(np.array(
                        (np.array([[extent.x, extent.y, extent.z, 1]]) @ _rotation_matrix)[:3]))
    ...
    def _get_2d_bb_baseline(self, obstacle, distance=2, cam='seg_front'):
        ...
        cords_x_y_z = np.array(self._world_to_sensor(
            trig_loc_world, self._get_sensor_position(cam)))
        ...
        if self._get_dist(cords_x_y_z[:, indices[0]], cords_x_y_z[:, indices[1]]) < self._get_dist(cords_x_y_z[:, indices[0]], cords_x_y_z[:, indices[2]]):
            cords = cords_x_y_z[:, [indices[0], indices[2]]
                                ] + np.array([[distance], [0], [0], [0]])
        else:
            cords = cords_x_y_z[:, [indices[0], indices[1]]
                                ] + np.array([[distance], [0], [0], [0]])
    ...
    def _baseline_to_box(self, baseline, cam, height=1):
        ...
        """
        cords_x_y_z = np.array(self._world_to_sensor(
            baseline, self._get_sensor_position(cam))[:3, :])

        cords = np.hstack((cords_x_y_z, np.fliplr(
            cords_x_y_z + np.array([[0], [0], [height]]))))
    ...
    def _change_seg_stop(self, seg_img, depth_img, stop_signs, cam, _region_size=6):
        ...
        for stop in stop_signs:
            ...
            _x = self._world_to_sensor(
                _trig_loc_world, self._get_sensor_position(cam))[0, 0]
            ...
            if _x > 0:  # stop is in front of camera
                ...
                if np.any(camera_bbox[:, 2] > 0):
                    ...
                    img = Image.new(
                        'L', (self._sensor_data['width'], self._sensor_data['height']), 0)
                ...
                if sensor_cords[0, i] < 0:
                    for j in _num_cords:
                        if sensor_cords[0, j] > 0:
                            _direction = sensor_cords[:,
                                                      i] - sensor_cords[:, j]
                            _distance = -sensor_cords[0, j] / _direction[0]
                            new_cord = sensor_cords[:, j] + \
                                _distance[0, 0] * _direction * 0.9999
                            modified_cords = np.hstack(
                                [modified_cords, new_cord]) if modified_cords.size else new_cord
                else:
                    modified_cords = np.hstack(
                        [modified_cords, sensor_cords[:, i]]) if modified_cords.size else sensor_cords[:, i]
