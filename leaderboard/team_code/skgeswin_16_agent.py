...
from skgeswin_16.model import skgeswin_16
from skgeswin_16.config import GlobalConfig
from skgeswin_16.data import scale_and_crop_image, scale_and_crop_image_cv, rgb_to_depth, swap_RGB2BGR
...
def get_entry_point():
    return 'skgeswin_16Agent'


class skgeswin_16Agent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        # 'rgb_left': deque(), 'rgb_right': deque(), 'rgb_rear': deque(),
        self.input_buffer = {'rgb': deque(), 'depth': deque(),
                             'gps': deque(), 'thetas': deque()}

        self.config = GlobalConfig()
        self.net = skgeswin_16(self.config, torch.device("cuda:0")
    ...
    def tick(self, input_data):
        ...
        depth = cv2.cvtColor(
            input_data['depth'][1][:, :, :3], cv2.COLOR_BGR2RGB)
    ...
    def run_step(self, input_data, timestamp):
        ...
        rgb = torch.from_numpy(scale_and_crop_image(Image.fromarray(
            tick_data['rgb']), scale=self.config.scale, crop=self.config.input_resolution)).unsqueeze(0)
        ...
        depth = torch.from_numpy(np.array(rgb_to_depth(scale_and_crop_image_cv(swap_RGB2BGR(
            tick_data['depth']), scale=self.config.scale, crop=self.config.input_resolution))))
        ...
        with torch.cuda.amp.autocast():
            pred_seg, pred_wp, psteer, pthrottle, pbrake, predl, pstops, pred_sc = self.net(
                self.input_buffer['rgb'], self.input_buffer['depth'], target_point, gt_velocity)
        mlp_steer = np.clip(psteer.cpu().data.numpy(), -1.0, 1.0)
        mlp_throttle = np.clip(pthrottle.cpu().data.numpy(),
                               0.0, self.config.max_throttle)
        # np.clip(pbrake.cpu().data.numpy(), 0.0, 1.0)
        mlp_brake = np.round(pbrake.cpu().data.numpy(), decimals=0)

        # pid_steer, pid_throttle, pid_brake, pid_metadata = self.net.pid_control(pred_wp, gt_velocity) #PID ONLY
        steer, throttle, brake, metadata = self.net.mlp_pid_control(
            pred_wp, gt_velocity, mlp_steer, mlp_throttle, mlp_brake, predl, pstops, CONTROL_OPTION)  # MIX MLP AND PID
        # if brake < 0.05: brake = 0.0
        # if throttle > brake: brake = 0.0

        self.control_metadata = metadata
        # tambahan metadata, replace value yang ada di fungsi control model
        self.control_metadata['car_pos'] = tuple(
            [float(tick_data['gps'][0]), float(tick_data['gps'][1])])
        self.control_metadata['next_point'] = tuple([float(tick_data['target_point'][0].cpu(
        ).data.numpy()), float(tick_data['target_point'][1].cpu().data.numpy())])

        control = carla.VehicleControl()
        control.steer = float(steer)  # pid_steer mlp_steer steer
        # pid_throttle mlp_throttle throttle
    ...
    def save(self, tick_data):
        ...
        Image.fromarray(tick_data['rgb']).save(
            self.save_path / 'rgb' / ('%06d.png' % frame))
        Image.fromarray(swap_RGB2BGR(tick_data['depth'])).save(
            self.save_path / 'depth' / ('%06d.png' % frame))
    ...
    def get_wp_nxr_frame(self):
        ...
        for i in range(1, self.config.pred_len+1):
            x_point = int(
                (frame_dim/2) + (self.control_metadata['wp_'+str(i)][0]*(frame_dim/2)/area))
            y_point = int(
                frame_dim - (self.control_metadata['wp_'+str(i)][1]*frame_dim/area))
            xy_arr = np.clip(
                np.array([x_point, y_point]), 0, frame_dim)  # constrain
        ...
        x_point = int(
            (frame_dim/2) + (self.control_metadata['next_point'][0]*(frame_dim/2)/area))
        y_point = int(
            frame_dim + (self.control_metadata['next_point'][1]*frame_dim/area))
        xy_arr = np.clip(np.array([x_point, y_point]),
                         0, frame_dim)  # constrain
        point_xy.append(xy_arr)
        return point_xy
    ...
    def save2(self, ss, sc):
        ...
        for i in range(self.config.pred_len):
            imgx2 = cv2.circle(imgx2, (wp_nxr_frame[i][0], wp_nxr_frame[i][1]), radius=2, color=(
                255, 255, 255), thickness=-1)

        # gambar juga next routenya, ada di element terakhir dalam list
        imgx2 = cv2.circle(
            imgx2, (wp_nxr_frame[-1][0], wp_nxr_frame[-1][1]), radius=4, color=(255, 255, 255), thickness=1)
