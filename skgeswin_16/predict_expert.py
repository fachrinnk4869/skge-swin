...
def test(data_loader, model, config, device):
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            total_metric = (1-metric_seg) + metric_wp + metric_str + \
                metric_thr + metric_brk + (1-metric_redl) + (1-metric_stops)
            ...
            pd.DataFrame(log).to_csv(os.path.join(
                save_dir, 'test_log_'+config.test_weather+'.csv'), index=False)
...
model = skgeswin_16(config, device).float().to(device)
...
dataloader_test = DataLoader(
    test_set, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)  # BS 1
