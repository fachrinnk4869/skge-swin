...
def BCEDice(Yp, Yt, smooth=1e-7):
    with torch.cuda.amp.autocast(enabled=False):
        Yp = Yp.float()  # Pastikan pakai float32
        Yt = Yt.float()
        Yp = Yp.view(-1)
        Yt = Yt.view(-1)
...
def renormalize_params_lw(current_lw, config):
    ...
    normalized_lws = [torch.cuda.FloatTensor([lw]).clone(
    ).detach().requires_grad_(True) for lw in new_lws]
...
def train(data_loader, model, config, writer, cur_epoch, device, optimizer, params_lw, optimizer_lw):
    ...
    for data in data_loader:
        ...
        # ambil yang terakhir aja #[-1]
        # ambil yang terakhir aja #[-1]
        fronts = data['fronts'].to(device, dtype=torch.float)
        seg_fronts = data['seg_fronts'].to(
            device, dtype=torch.float)  # ambil yang terakhir aja #[-1]
        depth_fronts = data['depth_fronts'].to(
            device, dtype=torch.float)  # ambil yang terakhir aja #[-1]
        target_point = torch.stack(data['target_point'], dim=1).to(
            device, dtype=torch.float)
        gt_velocity = data['velocity'].to(device, dtype=torch.float)
        gt_waypoints = [torch.stack(data['waypoints'][i], dim=1).to(
            device, dtype=torch.float) for i in range(config.seq_len, len(data['waypoints']))]
        ...
        with torch.cuda.amp.autocast():
            pred_seg, pred_wp, steer, throttle, brake, red_light, stop_sign, _ = model(
                fronts, depth_fronts, target_point, gt_velocity)
        ...
        total_loss = params_lw[0]*loss_seg + params_lw[1]*loss_wp + params_lw[2]*loss_str + \
            params_lw[3]*loss_thr + params_lw[4]*loss_brk + \
            params_lw[5]*loss_redl + params_lw[6]*loss_stops

        optimizer.zero_grad()

        if batch_ke == 0:  # batch pertama, hitung loss awal
            scaler.scale(total_loss).backward()  # ga usah retain graph
            ...
            if config.MGN:
                ...
                # retain graph karena graphnya masih dipakai perhitungan
                total_loss.backward(retain_graph=True)
                params = list(
                    filter(lambda p: p.requires_grad, model.parameters()))
                G0R = torch.autograd.grad(
                    loss_seg, params[config.bottleneck[0]], retain_graph=True, create_graph=True)
                G0 = torch.norm(G0R[0], keepdim=True)
                G1R = torch.autograd.grad(
                    loss_wp, params[config.bottleneck[1]], retain_graph=True, create_graph=True)
                G1 = torch.norm(G1R[0], keepdim=True)
                G2R = torch.autograd.grad(
                    loss_str, params[config.bottleneck[1]], retain_graph=True, create_graph=True)
                G2 = torch.norm(G2R[0], keepdim=True)
                G3R = torch.autograd.grad(
                    loss_thr, params[config.bottleneck[1]], retain_graph=True, create_graph=True)
                G3 = torch.norm(G3R[0], keepdim=True)
                G4R = torch.autograd.grad(
                    loss_brk, params[config.bottleneck[1]], retain_graph=True, create_graph=True)
                G4 = torch.norm(G4R[0], keepdim=True)
                G5R = torch.autograd.grad(
                    loss_redl, params[config.bottleneck[0]], retain_graph=True, create_graph=True)
                G5 = torch.norm(G5R[0], keepdim=True)
                G6R = torch.autograd.grad(
                    loss_stops, params[config.bottleneck[0]], retain_graph=True, create_graph=True)
                ...
                loss_hat_avg = (loss_seg_hat + loss_wp_hat + loss_str_hat + loss_thr_hat +
                                loss_brk_hat + loss_redl_hat + loss_stops_hat) / len(config.loss_weights)
                ...
                Lgrad = F.l1_loss(G0, C0) + F.l1_loss(G1, C1) + F.l1_loss(G2, C2) + F.l1_loss(
                    G3, C3) + F.l1_loss(G4, C4) + F.l1_loss(G5, C5) + F.l1_loss(G6, C6)
                ...
                # Scale loss before backward pass
                scaler.scale(Lgrad).backward()
                # Take the step with the scaled gradients
                scaler.step(optimizer_lw)
                lgrad = Lgrad.item()
        ...
        #     param.grad = param.grad.float()  # Ensure gradients are in FP32
...
def validate(data_loader, model, config, writer, cur_epoch, device):
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            loss_redl = F.l1_loss(red_light, gt_red_light)
            loss_stops = F.l1_loss(stop_sign, gt_stop_sign)
            total_loss = loss_seg + loss_wp + loss_str + \
                loss_thr + loss_brk + loss_redl + loss_stops
...
def main():
    ...
    model = skgeswin_16(config, device).to(device)
    model_parameters = filter(lambda p: p.requires_grad, model.parameters())
    params = sum([np.prod(p.size()) for p in model_parameters])
    print('Total trainable parameters: ', params)

    # OPTIMIZER
    optima = optim.AdamW(model.parameters(), lr=config.lr,
                         weight_decay=config.weight_decay)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optima, mode='min', factor=0.5, patience=3, min_lr=1e-6)
    ...
    dataloader_val = DataLoader(
        val_set, batch_size=config.batch_size, shuffle=False, num_workers=4, pin_memory=True)
    ...
    if not os.path.exists(config.logdir+"/trainval_log.csv"):
        ...
        params_lw = [torch.tensor([config.loss_weights[i]], dtype=torch.float32, device='cuda').clone(
        ).detach().requires_grad_(True) for i in range(len(config.loss_weights))]
        ...
        model.load_state_dict(torch.load(
            os.path.join(config.logdir, 'recent_model.pth')))
        optima.load_state_dict(torch.load(
            os.path.join(config.logdir, 'recent_optim.pth')))
        # set optima lw baru
        latest_lw = [float(log_trainval['lw_ss'][-1:]), float(log_trainval['lw_wp'][-1:]), float(log_trainval['lw_str'][-1:]), float(
            log_trainval['lw_thr'][-1:]), float(log_trainval['lw_brk'][-1:]), float(log_trainval['lw_redl'][-1:]), float(log_trainval['lw_stops'][-1:])]
        params_lw = [torch.cuda.FloatTensor([latest_lw[i]]).clone(
        ).detach().requires_grad_(True) for i in range(len(latest_lw))]
    ...
    while True:
        ...
        train_log, new_params_lw, lgrad = train(
            dataloader_train, model, config, writer, epoch, device, optima, curr_lw, optima_lw)
        val_log = validate(dataloader_val, model,
                           config, writer, epoch, device)
        if config.MGN:
            optima_lw.param_groups[0]['params'] = renormalize_params_lw(
                new_params_lw, config)  # harus diclone supaya benar2 terpisah
            print("total loss gradient: "+str(lgrad))
        # parameter acuan reduce LR adalah val_total_metric
        ...
        print('| t_total_l: %.4f | t_ss_l: %.4f | t_wp_l: %.4f | t_str_l: %.4f | t_thr_l: %.4f | t_brk_l: %.4f | t_redl_l: %.4f | t_stops_l: %.4f |' % (
            train_log['t_total_l'], train_log['t_ss_l'], train_log['t_wp_l'], train_log['t_str_l'], train_log['t_thr_l'], train_log['t_brk_l'], train_log['t_redl_l'], train_log['t_stops_l']))
        print('| v_total_l: %.4f | v_ss_l: %.4f | v_wp_l: %.4f | v_str_l: %.4f | v_thr_l: %.4f | v_brk_l: %.4f | v_redl_l: %.4f | v_stops_l: %.4f |' % (
            val_log['v_total_l'], val_log['v_ss_l'], val_log['v_wp_l'], val_log['v_str_l'], val_log['v_thr_l'], val_log['v_brk_l'], val_log['v_redl_l'], val_log['v_stops_l']))
        print('elapsed time: %.4f sec' % (elapsed_time))

        # save recent model
        torch.save(model.state_dict(), os.path.join(
            config.logdir, 'recent_model.pth'))
        torch.save(optima.state_dict(), os.path.join(
            config.logdir, 'recent_optim.pth'))

        # save model best only
        if val_log['v_total_l'] < lowest_score:
            print("v_total_l: %.4f < lowest sebelumnya: %.4f" %
                  (val_log['v_total_l'], lowest_score))
            print("model terbaik disave!")
            torch.save(model.state_dict(), os.path.join(
                config.logdir, 'best_model.pth'))
            torch.save(optima.state_dict(), os.path.join(
                config.logdir, 'best_optim.pth'))
        ...
        # early stopping
        if stop_count == 0:
            print("TRAINING BERHENTI KARENA TIDAK ADA PENURUNAN TOTAL LOSS DALAM %d EPOCH TERAKHIR" % (
                config.init_stop_counter))
            break  # loop
